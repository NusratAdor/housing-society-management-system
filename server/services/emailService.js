 ...
 * Sent after an admin confirms a gateway-verified payment (i.e. once
 * allocatePayment() has successfully committed — dues are already marked
 * Paid and a receipt already generated by the time this runs).
 * Fetches charge details from MongoDB to build the breakdown table.
 *
 * Called from: adminPaymentController.approvePayment — this is the only
 * call site in the codebase. The SSLCommerz IPN callback
 * (paymentController.paymentCallback) never sends this email; it only
 * marks a payment "verified" and stores the confirmed charge selection
 * for the admin to later approve.
 */
